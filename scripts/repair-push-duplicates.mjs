...
export function repairPushDuplicates() {
  ...
  if(!targetPaths.length){console.log("Duplicate repair: this push did not add or modify a discovery JSON file.");return}
  ...
  function updateRunObject(run,removed,survivors){
   ...
   if (typeof run.rejection_summary === "string") {
     run.rejection_summary=base?`${base} ${sentence}`:sentence;
   } else {
     // Structured rejection evidence is accepted by validate-run-logs.mjs.
     // Keep it verbatim; repair diagnostics belong beside it, never in its place.
     const notes = Array.isArray(run.duplicate_repair_notes) ? run.duplicate_repair_notes : [];
     run.duplicate_repair_notes = [...notes, sentence];
   }
  }
